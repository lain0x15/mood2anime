"use strict";
let animeIDs,animeIDsOffset,screenshot_offset;
...
function show_screenshot (offset){
  ...
  let slides = document.getElementsByClassName("anime_image");
  if (0 > screenshot_offset){
    screenshot_offset = slides.length - 1;
  } else if (slides.length <= screenshot_offset) {
    screenshot_offset = 0;
  }
  let active_slides = document.getElementsByClassName("anime_image_active");
...
function getAnimeByID (id){
  $.ajax({
    ...
    success: function(data){
      let anime = JSON.parse(data).anime;
      let old_innerContent = document.getElementById("innerContent");

      let innerContent = document.createElement('div');
      innerContent.setAttribute('id', 'innerContent');

      let anime_screenshots = document.createElement("div");
      anime_screenshots.setAttribute('id', 'anime_screenshots');

      let anime_title = document.createElement("div");
      anime_title.setAttribute('id', 'anime_title');

      let anime_info = document.createElement("div");
      anime_info.setAttribute('id', 'anime_info');

      let anime_genres = document.createElement("div");
      ...
      let anime_buttons = document.createElement("div");
      ...
      let url_mask = anime_page.replace(/0/, anime.url_name);
      let link_to_anime = document.createElement("a");
      ...
      anime.genre.forEach((element) => {
        ...
        let contentGenre = document.createElement("div");
      ...
      let anime_buttons_bottoms = document.createElement("div");
